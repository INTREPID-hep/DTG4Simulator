...
class EventAction : public G4UserEventAction
{
  public:
    ...
    void AddRadiatedEnergy(G4int trackID, G4double energy) const { fRadiatedEnergyMap[trackID] += energy; }
    void AddSecondaryCount(G4int trackID, G4int count) const { fSecondaryCountMap[trackID] += count; }

  private:
    const RunAction* fRunAction;
    
    // Radiation counters mapped by TrackID
    mutable std::map<G4int, G4double> fRadiatedEnergyMap;
    mutable std::map<G4int, G4int> fSecondaryCountMap;
